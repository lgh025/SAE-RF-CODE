...
for i=positions % for each column of the coding matrix
    ...
    for j=1:size(ECOC,1) % for each row of the coding matrix
        ...
        elseif ECOC(j,i)==0 % save the current class data in the second partition
            pos=find(data(:,size(data,2))==classes(j));
     ...
     try, % apply the base classifier over the current partition of classes
         P_train1=[ FirstSet;SecondSet];
         T_train1=[ ones(size(FirstSet,1),1);2*ones(size(SecondSet,1),1)];
         model = classRF_train(P_train1,T_train1, Parameters.ntree, Parameters.mtry);
         BER=sum(model.outcl~=T_train1);
%         Custom_classifier=feval(Parameters.base,FirstSet,SecondSet,Parameters.base_params);
    ...
    Classifiers{length(Classifiers)+1}.classifier=model; % save current classifier and data
